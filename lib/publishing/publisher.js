...
class MCPPublisher {
  ...
  validatePackage(packageData) {
    ...
    if (packageData.endpoint) {
      // Allow various formats: npm packages, git repos, local commands
      const validFormats = [
        /^npx\s+[\w@/-]+/, // npm packages
        /^uv\s+run\s+[\w-]+/, // uv packages
        /^node\s+[\w./]+/, // local node scripts
        /^python\s+[\w./]+/, // python scripts
        /^e14z\s+run\s+[\w-]+/, // e14z managed MCPs
        /^[\w-]+$/ // simple commands
      ];
      
      const isValid = validFormats.some(regex => regex.test(packageData.endpoint));
      if (!isValid) {
        errors.push('Invalid endpoint format. Use npm, uv, node, python, e14z run, or simple command format.');
      }
    }
  ...
  generateCleanCommand(endpoint) {
    ...
    const transformations = [
      ...
      { pattern: /^npm\s+run\s+([\w-]+)$/, replacement: 'npm run $1' },
      
      // Python packages
      { pattern: /^uv\s+run\s+([\w-]+)$/, replacement: 'uv run $1' },
      { pattern: /^pip\s+install\s+(.+)$/, replacement: 'pip install $1' },
      ...
      // Node scripts
      { pattern: /^node\s+([\w./]+)$/, replacement: 'node $1' },
      
      // Python scripts
      { pattern: /^python\s+([\w./]+)$/, replacement: 'python $1' }
    ];
