...
class ComprehensiveIntelligenceCollector {
  ...
  async collectIntelligence(packageInfo) {
    ...
    try {
      ...
      try {
        ...
        // Use auth hint for better detection strategy
        intelligence = await collector.collectIntelligence(packageInfo.name, authHint);
  ...
  transformToDatabase(intelligence, packageInfo, installResult) {
    ...
    return {
      ...
      description: intelligence.intelligentDescription || intelligence.business.valueProposition || packageInfo.description,
      ...
      credentials_needed: intelligence.auth.credentialsNeeded || [],
      required_env_vars: intelligence.auth.requiredEnvVars,
      optional_env_vars: intelligence.auth.optionalEnvVars || [],
      auth_summary: intelligence.auth.authSummary,
      setup_complexity: this.normalizeSetupComplexity(intelligence.auth.setupComplexity),
      setup_instructions: intelligence.auth.setupInstructions,
      ...
      // Business value (AI-enhanced)
      use_cases: intelligence.intelligentUseCases || intelligence.business.useCases,
      value_proposition: intelligence.business.valueProposition,
      ...
      // Search optimization (AI-enhanced)
      category: this.inferCategory(packageInfo, intelligence),
      tags: intelligence.intelligentTags || this.generateTags(packageInfo, intelligence),
  ...
  determineHealthStatus(intelligence) {
    ...
    const authMethods = intelligence.auth.methods || [];
    const hasProtocol = !!intelligence.protocol.version;
    const hasAuthErrors = intelligence.errors.patterns.some(error => 
    ...
    // If we couldn't establish a protocol connection at all
    if (!hasProtocol) {
      return 'down'; // Server won't start or connect - actually broken
    }
    
    // If server connected and we have clear auth requirements with proper error messages
    if (authRequired && hasAuthErrors && authMethods.length > 0) {
      // Server is working perfectly, just needs authentication
      // This is HEALTHY - it's doing exactly what it should do without credentials
      return 'healthy-requires-auth';
    }
    
    // If auth is required but we don't have clear error messages (edge case)
    if (authRequired && totalTools > 0 && workingTools === 0 && !hasAuthErrors) {
      // Unclear auth state - might be misconfigured
      return 'requires-auth';
    }
    
    // No auth required scenarios
    if (!authRequired) {
      if (totalTools === 0) {
        // MCP connected successfully but has no tools - healthy for resource/prompt MCPs
        return 'healthy';
      }
      
      if (workingTools === totalTools) {
        return 'healthy'; // All tools working perfectly
      } else if (workingTools > 0) {
        return 'degraded'; // Some tools working, some failing
      } else {
        return 'down'; // No tools working despite being available - actually broken
      }
    }
    
    // Auth required but no clear patterns - conservative assessment
    if (authRequired && totalTools > 0) {
      return 'requires-auth';
    }
  ...
  normalizeSetupComplexity(complexity) {
    // Ensure setup_complexity is one of the allowed values
    if (complexity === 'unknown' || !complexity) return 'simple';
    if (['simple', 'moderate', 'complex'].includes(complexity)) return complexity;
