        ...
        res.on('end', () => {
          try {
            ...
            if (latestVersion && packageInfo.versions[latestVersion]) {
              // Merge package-level data (like README) with version-level data
              const versionData = packageInfo.versions[latestVersion];
              const enrichedData = {
                ...versionData,
                // Override with package-level data when available
                readme: packageInfo.readme || versionData.readme,
                repository: packageInfo.repository || versionData.repository,
                homepage: packageInfo.homepage || versionData.homepage,
                // Keep package-level metadata
                time: packageInfo.time,
                maintainers: packageInfo.maintainers,
                license: packageInfo.license || versionData.license
              };
