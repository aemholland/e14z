...
class AgentOptimizedAnalyzer {
  ...
  generateServiceSpecificUseCases(name, description, allText) {
    ...
    for (const [pattern, cases] of Object.entries(servicePatterns)) {
      ...
      if (regex.test(allText) || regex.test(name)) {
        useCases.push(...cases);
        break; // Only match the most specific service
      }
    }
  ...
  async generateIntelligentDescription(packageInfo, npmDetails, githubInfo, webDocs, analysis) {
    ...
    const readme = githubInfo?.readme || '';
  ...
  /**
   * Use AI reasoning to synthesize a compelling, natural description
   */
  synthesizeIntelligentDescription(context) {
    ...
    // Detect the core purpose and write naturally
    let description = '';
    
    // Start with the core value proposition
    if (serviceType.isIntegration) {
      description = `Seamlessly integrate ${serviceType.service} with AI agents and automation workflows. `;
    } else if (serviceType.isDatabase) {
      description = `Execute intelligent database operations and queries through a unified MCP interface. `;
    } else if (serviceType.isFileSystem) {
      description = `Access and manipulate files and directories with AI-powered automation capabilities. `;
    } else if (serviceType.isAPI) {
      description = `Bridge external APIs and services for AI agents with robust data access patterns. `;
    } else {
      // Generic but intelligent opening
      description = `Empowers AI agents with ${this.extractCoreCapability(name, tools, capabilities)} through a streamlined MCP interface. `;
    }
    
    // Add specific capabilities
    if (tools.length > 0) {
      const toolCategories = this.categorizeMeaningfully(tools);
      description += `Features ${tools.length} specialized tools for ${toolCategories.join(', ')}, `;
    }
    
    // Add business value
    if (businessValue.length > 0) {
      description += `enabling ${businessValue.slice(0, 2).join(' and ')}. `;
    }
    
    // Add differentiation
    const differentiators = this.extractDifferentiators(readme, capabilities);
    if (differentiators.length > 0) {
      description += `${differentiators[0]} `;
    }
    
    // Add practical usage context
    if (useCases.length > 0) {
      const primaryUseCase = useCases[0];
      if (primaryUseCase && primaryUseCase.length > 20) {
        description += `Ideal for ${primaryUseCase.toLowerCase()}.`;
      }
    }
    
    // Clean up and ensure natural flow
    return this.polishDescription(description);
  }
