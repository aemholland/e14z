...
class PipPackageManager extends BasePackageManager {
  ...
  async findExecutable(packageInfo, cacheDir) {
    // 1. First check for console scripts in bin directory (most common for Python packages)
    const binDir = path.join(cacheDir, 'bin');
    try {
      const binFiles = await fs.readdir(binDir);
      
      // Look for executable that matches package name (with or without hyphens)
      const packageBaseName = packageInfo.name.replace(/_/g, '-'); // Convert underscores to hyphens
      const candidates = [
        packageBaseName,
        packageInfo.name,
        packageBaseName.replace(/^mcp-?/, ''), // Remove mcp prefix
        ...binFiles.filter(f => f.includes('mcp') || f.includes(packageBaseName))
      ];
      ...
      // No bin directory or no executables found, continue to other methods
    }
    
    // 2. Check for __main__.py module
    ...
    try {
      ...
      // Convert package name to valid Python module name (replace hyphens with underscores)
      const pythonModuleName = packageInfo.name.replace(/-/g, '_');
      
      return {
        command: process.platform === 'win32' ? 'python.exe' : 'python3',
        args: ['-c', `import ${pythonModuleName}; ${pythonModuleName}.main()`],
        type: 'python',
        env: { PYTHONPATH: cacheDir }
      };
    }
