...
class AutoInstaller {
  ...
  selectInstallMethod(mcp, options = {}) {
    ...
    if (mcp.auto_install_command) {
      ...
      if (command.startsWith('npx ') || command.includes('npm install')) {
        type = 'npm';
      } else if (command.startsWith('pipx ') || command.includes('pipx install') || command.startsWith('pip ') || command.includes('pip install')) {
        type = 'pipx'; // Convert old pip commands to pipx
      } else if (command.startsWith('cargo ') || command.includes('cargo install')) {
        type = 'cargo';
      } else if (command.startsWith('go ') || command.includes('go install')) {
        type = 'go';
      } else if (command.startsWith('e14z run ')) {
        type = 'e14z';
      }
      
      if (!type) {
        throw new Error(`Unsupported installation command: ${command}. Only npm, pipx, cargo, go, and e14z are supported.`);
      }
    ...
    // Enhanced priority order: npm, pipx, cargo, go, e14z
    const priorityOrder = ['npm', 'pipx', 'cargo', 'go', 'e14z'];
    ...
    if (mcp.endpoint) {
      ...
      } else if (mcp.endpoint.includes('pipx') || mcp.endpoint.includes('pip')) {
        return {
          type: 'pipx', // Convert old pip to pipx
          command: mcp.endpoint.replace('pip install', 'pipx install'),
          description: 'Parsed from endpoint (converted to pipx)'
        };
      } else if (mcp.endpoint.includes('cargo')) {
        return {
          type: 'cargo',
          command: mcp.endpoint,
          description: 'Parsed from endpoint'
        };
      } else if (mcp.endpoint.includes('go install')) {
        return {
          type: 'go',
          command: mcp.endpoint,
          description: 'Parsed from endpoint'
        };
      } else if (mcp.endpoint.includes('e14z run')) {
        return {
          type: 'e14z',
          command: mcp.endpoint,
          description: 'Parsed from endpoint'
  ...
  async executeFromSecureCache(slug, mcp, packageInfo, packageManager, cacheLocation, options = {}) {
    try {
      ...
      } else if (execCommand.type === 'python' || execCommand.type === 'python_script') {
