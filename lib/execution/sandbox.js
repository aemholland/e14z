...
class CommandSanitizer {
  ...
  sanitizeArgs(args, context = {}) {
    ...
    return args.map(arg => {
      ...
      // For legitimate Python MCP servers, allow necessary Python syntax
      const isPythonScript = context.isPython && context.isLegitimateScript && args.includes('-c');
      
      // Check for dangerous patterns
      for (const pattern of this.dangerousPatterns) {
        // Skip semicolon pattern only for verified Python MCP scripts
        if (isPythonScript && pattern.source === ';') {
          continue;
        }
        if (pattern.test(arg)) {
      ...
      for (const char of this.dangerousChars) {
        // Skip specific characters only for verified Python MCP scripts
        if (isPythonScript && (char === ';' || char === '(' || char === ')')) {
          continue;
        }
   ...
   */
  sanitizeEnvironment(env, context = {}) {
    const sanitizedEnv = {};
    ...
    // PYTHONPATH is generally dangerous, but allow for legitimate Python MCP servers only
    if (!context.isPython || !context.isLegitimateScript) {
      dangerousEnvVars.push('PYTHONPATH');
    }
...
class ProcessSandbox {
  ...
  async executeInSandbox(command, args, options = {}) {
    ...
    const isPython = command.includes('python') || command.endsWith('python3');
    const isLegitimateScript = command.includes('mcp-server-') || command.includes('@modelcontextprotocol');
    const sanitizedArgs = this.sanitizer.sanitizeArgs(args || [], { isPython, isLegitimateScript });
    const sanitizedEnv = this.sanitizer.sanitizeEnvironment(options.env, { isPython, isLegitimateScript });
...
class SecureExecutor {
  ...
  async execute(command, args, options = {}) {
    if (!this.enableSandboxing || this.securityLevel === 'minimal') {
      ...
      const isPython = command.includes('python') || command.endsWith('python3');
      const isLegitimateScript = command.includes('mcp-server-') || command.includes('@modelcontextprotocol');
      const sanitizedArgs = sanitizer.sanitizeArgs(args || [], { isPython, isLegitimateScript });
      const sanitizedEnv = sanitizer.sanitizeEnvironment(options.env, { isPython, isLegitimateScript });
