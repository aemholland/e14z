...
class AuthManager {
  ...
  async startGitHubAuth() {
    ...
    // Check if GitHub client ID is configured (client_secret not needed for device flow)
    const clientId = process.env.E14Z_GITHUB_CLIENT_ID || 'Ov23libHRlbXdOXI0CHi';
    
    if (!clientId || clientId === 'NEW_CLIENT_ID_HERE') {
      throw new Error('GitHub authentication is not configured. Please contact the E14Z team or set E14Z_GITHUB_CLIENT_ID environment variable.');
    }
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        const tokenResponse = await fetch('https://github.com/login/oauth/access_token', {
          ...
          body: JSON.stringify({
            client_id: process.env.E14Z_GITHUB_CLIENT_ID || 'Ov23libHRlbXdOXI0CHi',
        ...
        try {
          ...
          if (!tokenResponse.ok && tokenResponse.status !== 400) {
            // Only log unexpected errors, not normal polling responses
          }
        } catch (error) {
